...
<div class="app-main__outer">
    <div class="app-main__inner">
        ...
        <div class="tabs-animation">
            <div class="card mb-3">
                <div class="card-header-tab card-header justify-content-end mr-3">
                    <div class="  justify-content-end">
                        <a class="btn btn-success" href="{{url('prosestindakan')}}" style="right: 0px !important;"><i class="fa fa-refresh mr-1" aria-hidden="true"></i> Proses</a>
                    </div>
                </div>
                                ...
                                <div class="tab-pane active" id="catatan" role="tabpanel">
                                    <form id="inputCatatan">
                                        <div class="row align-items-center form-group">
                                            ...
                                            <input name="alergi" id="exampleEmail22" placeholder="........." type="text" class="form-control col-6 col-lg-9 col-md-8">
                                        </div>
                                        <div class="row">
                                            <div class="form-group col-4 col-lg-2">
                                                <label for="tinggibadan" class="label-control f-75">Tinggi Badan</label>
                                                <input type="text" class="msgtinggibadan form-control">
                                            </div>
                                            <div class="form-group col-4 col-lg-2">
                                                <label class="label-control f-75">Berat Badan</label>
                                                <input type="text" name="beratbadan" class="msgberatbadan form-control">
                                            </div>
                                            <div class="form-group col-4 col-lg-2">
                                                <label for="tekanandarah" class="label-control f-75">Tekanan Darah</label>
                                                <input type="text" name="tekanandarah" class="msgtekanandarah form-control">
                                            </div>
                                            <div class="form-group col-5 col-lg-3">
                                                <label for="goldar" class="label-control f-75">Golongan Darah</label>
                                                <select name="goldar" class="msggoldar">
                                                    <option hidden disabled selected>Pilih golongan darah</option>
                                        ...
                                        <div class="form-group">
                                            <input type="text" name="catatan" class="msgcatatan" id="ckeditor4">
                                        </div>
                                        <button class="btn btn-primary resetFalse" type="submit">Simpan Catatan</button>
...
<script type="text/javascript">
    ...
    $(document).on('click', '.nextket', function() {
        if (this.dataset.popup != "false") {
            c('asdok')
            ketshow()
            $('.pilih-diag').val(this.dataset.kode);
        } else {
            c(this.dataset.add + '/' + this.dataset.kode)
            doReq(this.dataset.add + '/' + this.dataset.kode, null, (res) => {
                $(this).next().html(res.one);
                if (res.status) {
                    $(res.have).html(res.data);
                }
            })
    ...
    $(document).on('keypress', '.changeresep', function() {
        const p = $(this).closest('tr');
        p.find('.savedata').show();
    })
    ...
    $('.goldar').select2();
    CKEDITOR.instances["ckeditor4"].on('change', function() {
        $('#ckeditor4').val(this.getData());
    });
    $(document).on('click', '.savedata', function() {
        const p = $(this).closest('tr'),
            k = p.find('.kod').text(),
            qty = p.find('input[name="qty"]').val(),
            ket = p.find('input[name="ket"]').val();
        doReq('editresep', {
            _token: tkn(),
            data: [k, qty, ket]
        }, (res) => {
